...
import cv2
import numpy as np  # Agregado para el procesamiento de imágenes comprimidas
import threading
...
class TopicMonitorNode(Node):
    def __init__(self, gui_callback):
        ...
        # Topics de imágenes comprimidas
        image_topics = ['/debug_image/compressed', '/debug_image_sign/compressed', '/debug_image_traffic/compressed']
        data_topics = [
        ...
        # Suscripción a topics de imágenes comprimidas
        for topic in image_topics:
            self.create_subscription(CompressedImage, topic, self.compressed_image_callback(topic), 10)

        for topic, msg_type in data_topics:
            self.create_subscription(msg_type, topic, self.data_callback(topic), 10)

    def compressed_image_callback(self, topic_name):
        def callback(msg):
            try:
                # Convertir imagen comprimida a OpenCV
                ...
                if cv_image is not None:
                    ...
                    self.get_logger().error(f"Failed to decode compressed image from {topic_name}")
            except Exception as e:
                self.get_logger().error(f"Compressed image conversion failed for {topic_name}: {e}")
        return callback
...
class ROSMonitorGUI:
    def __init__(self):
        ...
        # Actualizar los topics de imagen para usar nombres comprimidos
        self.image_topics = {
            '/debug_image/compressed': {'title': '🎥 Cámara Principal', 'icon': '🎯'},
            '/debug_image_sign/compressed': {'title': '🚸 Detección Señales', 'icon': '⚠️'},
            '/debug_image_traffic/compressed': {'title': '🚦 Semáforos', 'icon': '🔴'}
        }
